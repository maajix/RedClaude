...
-- prototype/scope-proxy/run_tls_local.sh re-run here): behind the interception
...
--   3. A separate MEASUREMENT lane, not a separate egress path. `lane =
--      'transport'` is the runtime's own unintercepted handshake, taken through
--      the same scope decision, the same per-target concurrency slot and the
--      same token bucket as agent traffic (measured in the proxy prototype).
--      One egress path survives; what changes is who terminates the TLS.
...
-- The measurement lane. Ticket 14 proved containment over plaintext and handed
-- TLS here; its topology is one proxy listener per program on that program's
-- lane address, and this lane does not add a listener -- it is the same proxy
-- process opening the connection itself, with the target's real certificate
-- chain verified against the SYSTEM trust store rather than the run CA.
ALTER TABLE receipts DROP CONSTRAINT receipts_lane_check;
ALTER TABLE receipts ADD CONSTRAINT receipts_lane_check
    CHECK (lane IN ('agent','replay','proxy_internal','control','transport'));
...
-- chain-verified, hostname-verified handshake on the measurement lane.
...
ALTER TABLE receipts
    -- The measurement lane is by definition not intercepted, has no agent-side
    -- handshake to describe, and is in scope like anything else.
    ADD CONSTRAINT receipts_transport_lane_shape CHECK (
        lane <> 'transport' OR (
            intercepted = false
    ...
    -- Ticket 13's rule, extended: the measurement lane is egress, so served
    -- egress on it names a tool run too. The probe is a `tool_runs` row with
    -- transport = 'runtime' and tool_use_id NULL.
    ADD CONSTRAINT receipts_served_transport_needs_tool_run CHECK (
        NOT (lane = 'transport' AND decision = 'allowed' AND tool_run_id IS NULL)),
...
--   probe_only  a claim is possible, but only from a measurement-lane receipt,
--               and only over the listed fields.
...
COMMENT ON TABLE transport_makeability IS
 ...
 'from which lane, and over which receipt fields. Read by '
...
INSERT INTO transport_makeability (property_class, makeability, reason, allowed_fields) VALUES
 ('transport.tls_configuration', 'probe_only',
  ...
  'client h2 while the origin spoke http/1.1 only. Requires a measurement-lane '
  'receipt.',
 ...
 ('transport.certificate_trust', 'probe_only',
  ...
  'measured. Requires a measurement-lane receipt, whose chain is verified '
 ...
 ('transport.datagram_transport', 'unmakeable',
  ...
  'from a receipt is a fact about the proxy. The measurement lane does not fix '
  'this either -- it speaks TLS over TCP.',
  '{}');
...
BEGIN
    ...
    IF NOT r.transport_citable THEN
        RAISE EXCEPTION
          'transport observation refused: receipt % is lane=%, intercepted=%, '
          'divergence=%. Its TLS parameters are the proxy''s, not the target''s.',
          r.label, r.lane, r.intercepted, coalesce(r.transport_divergence, '{}')
          USING ERRCODE = 'check_violation';
    END IF;
