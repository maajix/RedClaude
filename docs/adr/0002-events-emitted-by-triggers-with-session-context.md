...
Every state-changing write appends exactly one event row (ADR 0001), and that
event row is written by an `AFTER INSERT OR UPDATE` trigger on the table, not by
the application code performing the write. The context a trigger cannot see —
`actor_kind`, `agent_run_id`, `task_id`, `caused_by_event_id`, `trace_id` —
arrives through `SET LOCAL` session settings that the runtime's connection helper
sets before any write. The trigger raises when `app.actor_kind` is unset.
