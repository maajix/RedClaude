...
BEGIN
    ...
    RETURN QUERY SELECT 'uuidv7_is_builtin'::text,
        coalesce((SELECT oid < 16384 FROM pg_proc WHERE proname = 'uuidv7' AND pronargs = 0), false),
        'uuidv7 oid = ' || coalesce((SELECT oid::text FROM pg_proc WHERE proname='uuidv7' AND pronargs=0), '<absent>');
