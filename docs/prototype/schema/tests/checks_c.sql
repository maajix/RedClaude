...
DECLARE p uuid := '4c000000-0000-7000-8000-00000000c001';
        ok boolean := false;
        note text := 'purge probe did not complete';
BEGIN
    BEGIN
        ...
        ok := true;
        note := 'purged with a scope version present';
    ...
    VALUES ('M04 a program carrying a scope version can be purged', 'probe', ok, note);
END $outer$;
...
DECLARE p uuid := '4c000000-0000-7000-8000-00000000c002';
        ...
        ok boolean := false;
        note text := 'guard probe did not complete';
BEGIN
    BEGIN
        ...
        note := 'purge blocked=' || blocked || ' check named it=' || named;
        RAISE EXCEPTION 'T_ROLLBACK';
    EXCEPTION WHEN OTHERS THEN
        IF SQLERRM <> 'T_ROLLBACK' THEN
            ok := false;
...
DECLARE p uuid := '4c000000-0000-7000-8000-00000000c006';
        ...
        ok boolean;
BEGIN
    -- No exception subtransaction here: its subxid would make the xmin-based
    -- checker report the fixture itself as an unaccounted write.
    ...
    SELECT r, p, 'orchestrator', ar.model, ar.effort, ar.mission_packet, 'session'
    ...
    SELECT count(*), string_agg(problem || ': ' || detail, '; ')
      INTO orphans, problems FROM check_event_log_integrity(p);
    ok := orphans = 0
          AND EXISTS (SELECT 1 FROM agent_sessions x
                       WHERE x.id = s AND x.unbound_at IS NOT NULL)
          AND EXISTS (SELECT 1 FROM events e
                       WHERE e.subject_id = s AND e.type = 'session.bound');
    INSERT INTO t.results (id, kind, pass, note)
    VALUES ('M06 resume unbinds the session instead of orphaning session.bound',
            'probe', ok, 'integrity problems after resume = ' || orphans ||
                         coalesce(': ' || problems, ''));
...
-- Scope version is now intentionally on the registered read surface. Its grant
-- is therefore explicit rather than inherited from the table.
SELECT t.expect_true('M10 scope_version is readable only through the registry',
  $$SELECT EXISTS (SELECT 1 FROM state_read_surface
                    WHERE table_name = 'programs' AND column_name = 'scope_version')
        AND has_column_privilege('rk2_state','programs','scope_version','SELECT')
...
DECLARE reach boolean; before bigint; after_grant bigint;
        ok boolean := false;
        note text := 'grant probe did not complete';
BEGIN
    BEGIN
        ...
        ok := before = 0 AND NOT reach AND after_grant > 0;
        note := 'silent before=' || before || ' new column readable=' || reach
    ...
    END;
    INSERT INTO t.results (id, kind, pass, note)
    VALUES ('M11 a new column is unreadable and a relation grant is caught',
            'probe', ok, note);
END $outer$;
...
DECLARE missing bigint; healed integer; after_heal bigint;
        ok boolean := false;
        note text := 'RLS creation probe did not complete';
BEGIN
    BEGIN
        CREATE TABLE drift_probe (
            id uuid PRIMARY KEY DEFAULT uuidv7(),
            program_id uuid NOT NULL REFERENCES programs(id) ON DELETE CASCADE);
        ALTER TABLE drift_probe OWNER TO rk2_owner;
        ...
        ok := missing = 3 AND healed >= 3 AND after_heal = 0;
        note := 'reported=' || missing || ' created=' || healed || ' after=' || after_heal;
        RAISE EXCEPTION 'T_ROLLBACK';
    EXCEPTION WHEN OTHERS THEN
        IF SQLERRM <> 'T_ROLLBACK' THEN
            ok := false;
    ...
    END;
    INSERT INTO t.results (id, kind, pass, note)
    VALUES ('M13 a table added without RLS is named, then healed by the finalizer',
            'probe', ok, note);
END $outer$;
...
DECLARE n bigint;
        ok boolean := false;
        note text := 'RLS disable probe did not complete';
BEGIN
    BEGIN
    ...
    END;
    INSERT INTO t.results (id, kind, pass, note)
    VALUES ('M14 RLS switched off on a scoped table is named', 'probe', ok, note);
END $outer$;
...
DECLARE n bigint;
        ok boolean := false;
        note text := 'checker registration probe did not complete';
BEGIN
    BEGIN
    ...
    END;
    INSERT INTO t.results (id, kind, pass, note)
    VALUES ('M15 a new checker with no standing_checks row is named', 'probe', ok, note);
END $outer$;
