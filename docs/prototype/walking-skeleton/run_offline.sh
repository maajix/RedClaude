...
(cd "$PROTOTYPE/sdk-auth-probe" && python3 -m unittest -v test_auth_resolution.py)
...
if command -v gitleaks >/dev/null 2>&1; then
    gitleaks dir --no-banner --redact=100 "$PROTOTYPE/sdk-auth-probe/evidence"
    for path in "$HERE"/*.py "$HERE"/*.sh "$HERE"/*.md \
                "$PROTOTYPE/schema/migrations/20260808T"*.sql \
                "$PROTOTYPE/schema/tests/capability_receipts.sql" \
                "$PROTOTYPE/schema/tests/startup_refusal.sql"; do
