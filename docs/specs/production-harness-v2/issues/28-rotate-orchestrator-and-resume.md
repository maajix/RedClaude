...
A session copies its three when it opens, along with the weights version it
...
than sent. `byte_ceiling` is `min(bytes, tokens * 4)`, so both configured limits
bind exactly. Every section states its own omission, because a subtraction a
...
Slate -- so one model is not handed two copies of the same list.

The standing checks are read inside the compile rather than carried from the
pass's own gate. The gate ran before the scheduler did, so quoting it would put
a sentence about an earlier moment inside a document claiming to describe this
one. The cost is one extra `run_standing_checks()` per pass.
...
`tests/test_capsule.py` (27 tests) is the document -- what the compile asks,
where a digest comes from, what a tight ceiling omits and what a malformed
capsule does when the child reads it. `CapsuleTest` in `tests/test_execution.py`
is the seam. `OrchestratorRotationTest` in `tests/test_database.py` is the
campaign on live rows: one Program per way of ending one, plus the capsule
compiled twice by two connections that share nothing, which is criterion 5 --
two runtimes, one moment, one document.
