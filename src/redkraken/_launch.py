...
DESCRIPTIONS = {
    ...
    "submit_mission_result": (
        ...
        "Four of those fields decide which strategy the next run gets handed, so "
        "leaving one out is not caution: an application with no kind, an endpoint "
        "with no auth_required, and an endpoint with no parameters between them "
        "silence about half the catalogue, and the hunt that follows runs under no "
        "Playbook at all. State kind on every application and auth_required on "
        "every endpoint you actually observed the answer for, and propose the "
        "parameters a route accepts as parameter entities under it. Where you did "
        "not observe the answer, leave the field out rather than guessing: an "
        "unknown value is a fact the selection can act on and a wrong one is "
        "not.\n\n"
