-- ---------------------------------------------------------------------------
-- an_evidence_edge_is_read_where_the_child_wrote_it.sql   (ticket 155)
--
-- `rk2hunt14` proposed three claims and lost all three to `no_support`. None of
-- them was unsupported. Each one carried its evidence edge inside itself:
--
--     {"ref": "h_hdr",
--      "evidence": [{"observation_ref": "o_hdr", "polarity": "supports", ...}],
--      "rationale": {...}, "statement": "...", "subject_label": "APP2",
--      "property_class": "transport.header_policy"}
--
-- Pass 2 of the promotion reads `payload -> 'evidence'` and nothing else, so it
-- walked an empty array, wrote no edges, and pass 3 refused every survivor for
-- having none.
--
-- The run that worked differs in placement and in nothing else:
--
--     run       | claims | edges at the top | edges inside a claim | survived
--     rk2hunt13 | 5      | 2, 4, 4, 3       | 0                    | 2
--     rk2hunt14 | 3      | 0                | one per claim        | 0
--
-- No code changed between them. Both the mission text and the tool description
-- ask for "at least one evidence edge naming it" and neither says at which
-- level, so which way a run goes is a property of the model and not of this
-- harness -- and the losing way is silent, because the drop is written after
-- the run has ended.
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION rk2_promote_hypotheses(uuid, jsonb, jsonb, integer) IS
  'Ticket 33''s three passes, with ticket 155''s lift: the edges of a proposal are its top-level `evidence` array together with the edges written inside the claims themselves, each of those naming the claim it was written in unless it named one of its own. A claim is graded on whether it cited support, not on where the citation was put.';
