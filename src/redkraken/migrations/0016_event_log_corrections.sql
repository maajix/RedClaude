...
--     ENABLE ALWAYS triggers, so a restore either declares an actor with
--     set_actor() or runs as a role allowed to turn the triggers off. The
--     supported restore does neither: a full archive recreates the triggers
--     after it has loaded the rows, so nothing emits a second time.
--
-- The role itself is provisioning, not migration: it is cluster-global, and
-- rk2_migrate has no CREATEROLE precisely so that a migration cannot mint one.
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'rk2_runtime') THEN
        RAISE EXCEPTION 'role rk2_runtime missing'
