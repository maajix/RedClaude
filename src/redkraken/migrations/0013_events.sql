...
-- ---------------------------------------------------------------------------
-- The actor context (decision 9)
-- ---------------------------------------------------------------------------
-- Every write declares who is writing, and this is the only supported way to
-- declare it.
--
-- The prototype declared it with a bare `SET app.actor_kind`, which is SESSION
-- scoped: one statement at connect time satisfied the emitter for the life of a
-- pooled connection, so an event recorded whoever had last thought to set it --
-- possibly a different run, in a different transaction, hours earlier. Nothing
-- failed; the attribution was simply wrong.
--
-- So the declaration is transaction-local AND says which transaction it was
-- made in. `emit_event()` compares that against the transaction actually
-- writing. A session-wide declaration then does not decay quietly one event at
-- a time: it stops the next transaction's first write, naming both ids.
CREATE FUNCTION set_actor(p_kind text, p_id text DEFAULT NULL) RETURNS void
LANGUAGE plpgsql AS $$
BEGIN
    -- pg_current_xact_id() assigns a real transaction id if this transaction
    -- has none yet, which is exactly the intent: a caller declaring an actor is
    -- about to write, and a transaction that never writes never reaches the
    -- emitter to be checked.
    PERFORM set_config('app.actor_kind', p_kind, true),
            set_config('app.actor_id', coalesce(p_id, session_user), true),
            set_config('app.actor_xact', pg_current_xact_id()::text, true);
END $$;

COMMENT ON FUNCTION set_actor(text, text) IS
    'Declares the actor for THIS transaction. Transaction-local and bound to '
    'the current transaction id, so a session-wide SET cannot attribute a '
    'later transaction''s writes.';
...
BEGIN
    ...
    -- decision 9: set_actor() is the only way to write. Not setting the actor
    -- is an error, never a silently unattributed event.
    ...
    END IF;
    -- ...and declares it in THIS transaction. Without this, one declaration on
    -- a pooled connection attributes every later transaction that reuses it.
