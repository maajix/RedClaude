...
LANGUAGE sql STABLE AS $$
  ...
  UNION ALL
    ...
    SELECT 'state_missing_registered_column', s.table_name || '.' || s.column_name,
           'state_read_surface names it but rk2_state cannot read it; run `rk db migrate`'
      FROM state_read_surface s
      JOIN pg_class c ON c.relname = s.table_name
...
COMMENT ON TABLE standing_checks IS
  'Every invariant the corpus asserts, run at the end of every `rk db migrate`. A migration that adds a check_ function and no row here fails check_state_grants''s sibling rule below.';
