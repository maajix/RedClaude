...
--
-- Receipts gain a `purpose`. Ticket 21 measured the agent's own control-plane
...
-- rate bucket -- so they are marked, rather than left to a rule in a policy no
-- program wrote.
--
-- The prototype marked them by adding a fourth Lane value, 'control'. This
-- corpus does not. Lane answers exactly one question -- which party caused the
-- request -- and it stays closed at `agent`, `replay` and `proxy_internal`,
-- because that is the question a citation asks when it decides whether an
-- Observation may rest on the receipt. Control-plane traffic is not a fourth
-- party; it is the harness acting as a client on its own behalf, which is what
-- `proxy_internal` already says. What it needs is an answer to a different
-- question -- why the request was made -- so it gets a different column.
--
...
ALTER TABLE receipts ADD COLUMN purpose text NOT NULL DEFAULT 'target_traffic';
ALTER TABLE receipts ADD CONSTRAINT receipts_purpose_check
    CHECK (purpose IN ('target_traffic','control_plane'));
-- A default here, and none on scope_class below, because the two carry
-- different kinds of silence: an unclassified scope is a decision nobody made,
-- while a request that claimed no special purpose is the program's own traffic
-- by construction -- that is what the proxy is in front of.
--
-- The two columns cannot disagree about who acted. Only the harness's own
-- client has a purpose other than the target's traffic; an agent-lane or
-- replay-lane request is the program's, whatever it says about itself.
ALTER TABLE receipts ADD CONSTRAINT receipts_purpose_is_proxy_internal
    CHECK (purpose = 'target_traffic' OR lane = 'proxy_internal');
...
ALTER TABLE receipts ADD CONSTRAINT receipts_control_plane_has_no_policy
    CHECK ((purpose = 'control_plane') = (scope_version IS NULL));
ALTER TABLE receipts ADD CONSTRAINT receipts_control_plane_class
    CHECK ((purpose = 'control_plane') = (scope_class = 'control_plane'));
