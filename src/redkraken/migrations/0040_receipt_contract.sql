...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'rk2_proxy') THEN
        RAISE EXCEPTION 'role rk2_proxy missing; run `rk db provision`';
    END IF;
...
COMMENT ON FUNCTION write_blocked_receipt(uuid,jsonb,text) IS
  'Writes only blocked receipts, on the agent lane or the harness''s own control-plane purpose; authority fields are derived and a valid capability is used only for agent attribution.';
